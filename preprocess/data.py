    ...
    def __iter__(self):
        for line in open(self.__file, 'r'):
            ...
            if self.__mode == 'raw_token':
                ...
                q1_tokens = [float(word2hash(token, self.voc_dict)) for token in q1]
                q2_tokens = [float(word2hash(token, self.voc_dict)) for token in q2]
                yield q1_tokens, q2_tokens
        ...
        for line in open(self.__file, 'r'):
            ...
            if self.__mode in ['raw', 'structure']:
                ...
                if self.__mode in ['raw_token', 'proc_token']:
                    ...
                    q_tokens = [float(word2hash(token, self.voc_dict)) for token in q_tokens]
                    a_tokens = [float(word2hash(token, self.voc_dict)) for token in a_tokens]
